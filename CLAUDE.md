...
**Orphan cleanup:** During update, orphan candidates are collected from two sources: (1) files tracked in the old manifest, and (2) files found on disk in managed directories (`agents/`, `rules/`, `commands/`, `skills/`, `memory/`, `hooks/`) that have a corresponding snapshot entry (proving they were previously deployed by the kit). Candidates absent from the new desired managed set (via `desired_managed_files_json()`) are detected as orphans and removed. User-created files (no manifest entry AND no snapshot) are never flagged. User-modified orphans (snapshot differs from current) and files with no snapshot baseline prompt for confirmation in interactive mode; non-interactive mode preserves them. Empty parent directories are cleaned up via `_cleanup_empty_dirs()`. Snapshot entries for removed orphans are also deleted. This ensures files deleted from the kit do not persist in `~/.claude/` across updates. Legacy `AGENTS.md` cleanup is excluded from orphan detection (handled by dedicated code).
...
- External side effects (shell RC, plugins, Codex CLI, Claude CLI) are individually guarded and logged as `[WOULD RUN]` entries via `_dryrun_log()`
...
- **Codex CLI scope**: Codex CLI is invoked via `codex exec` command delegation (not MCP).
